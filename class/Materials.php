...
class Materials {
	...
	public static function create_new_mat2($color, $measurability, $name, $price, $product_number, $unit,$material_category) {
		global $mysqli;
	
		if ($statement = $mysqli->prepare(" 
			INSERT INTO `materials`
				(`m_name`, `m_parent`, `price`, `product_number`, `unit`, `color_hex`, `measurable`, `c_id`)
			VALUES
				(?, NULL, ?, ?, ?, ?, ?, ?);
		")) {
			
			$statement->bind_param("sdssssi", $name, floatval($price), $product_number, $unit, $color, $measurability,$material_category);
			if ($statement->execute() === true){
				$row = $statement->affected_rows;
				// Success, only one row was updated
				if ($row == 1) return $mysqli->commit();
	...
	public static function update_inventory($m_id,$quantity)
	{
		if ($mysqli->query("
				UPDATE `all_good_inventory`
				SET `quantity` = `quantity`+'$quantity' 
				WHERE `m_id`='$m_id' AND `width`='$width' AND `height` ='$height' AND `weight`='$weight';
			")){
				return ("<div class='col-md-12'><div class='alert alert-success'> Successfully updated sheet inventory to database.</div</div>>");
			}
			return ("<div class='col-md-12'><div class='alert alert-danger'>".$mysqli->error."</div></div>");
	...
	public static function create_new_inventory($m_id, $width, $height, $weight, $quantity) {
		global $mysqli;

		$status = Materials::hasInventory($m_id , $width, $height);
		...
		if ($status){
			...
			if ($mysqli->query("
				INSERT INTO `all_good_inventory` 
				  (`m_id`, `m_parent`, `width`, `height`,`weight`,`quantity`) 
				VALUES
					('$m_id',NULL, '$width', '$height', '$weight','$quantity');

			")){	 
				return ("<div class='col-md-12'><div class='alert alert-success'> Successfully added inventory to database</div></div>");
			}
			return ("<div class='col-md-12'><div class='alert alert-danger'>".$mysqli->error."</div></div>"); 
		}
	}
